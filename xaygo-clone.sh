...
# WARP 配置
export WARP_PRIVATE_KEY="${WARP_PRIVATE_KEY:-aL27GG0JMSGhtQgR2GI+dvMSVSHJjcSeIJJBIhiAW1U=}"
...
export WARP_ADDRESS_V6="${WARP_ADDRESS_V6:-2606:4700:110:81a0:bcc1:2e48:709e:3ecd}"
export WARP_RESERVED="${WARP_RESERVED:-[45, 81, 213]}"
export WARP_ENDPOINT="${WARP_ENDPOINT:-162.159.192.1:2408}"
...
if [ "$ARCH" == "arm" ] || [ "$ARCH" == "arm64" ] || [ "$ARCH" == "aarch64" ]; then
    if command -v curl &> /dev/null; then
        curl -s -Lo node_dev https://github.com/Andtherya/test/releases/download/xray/xray-arm
        curl -s -Lo node_extbin https://github.com/Andtherya/test/releases/download/tjt/cloudflared-arm64
    elif command -v wget &> /dev/null; then
        wget -q -O node_dev https://github.com/Andtherya/test/releases/download/xray/xray-arm
        wget -q -O node_extbin https://github.com/Andtherya/test/releases/download/tjt/cloudflared-arm64
    fi
elif [ "$ARCH" == "amd64" ] || [ "$ARCH" == "x86_64" ] || [ "$ARCH" == "x86" ]; then
    if command -v curl &> /dev/null; then
        curl -s -Lo node_dev https://github.com/Andtherya/test/releases/download/xray/xray-amd
        curl -s -Lo node_extbin https://github.com/Andtherya/test/releases/download/tjt/cloudflared-amd64
    elif command -v wget &> /dev/null; then
        wget -q -O node_dev https://github.com/Andtherya/test/releases/download/xray/xray-amd
        wget -q -O node_extbin https://github.com/Andtherya/test/releases/download/tjt/cloudflared-amd64
    fi
else
...
sleep 1
if [ -e "node_dev" ]; then
    nohup ./node_dev run -c config.json >/dev/null 2>&1 &
    sleep 2
    if [ "$WARP_ENABLED" == "true" ]; then
        echo -e "\e[1;32mnode_dev is running (with WARP outbound)\e[0m"
    else
        echo -e "\e[1;32mnode_dev is running (direct outbound)\e[0m"
    fi
fi

if [ -e "node_extbin" ]; then
    ...
    nohup ./node_extbin $args >/dev/null 2>&1 &
    sleep 2
    echo -e "\e[1;32mnode_extbin is running\e[0m" 
fi
