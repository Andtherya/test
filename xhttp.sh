...
# 构建 VLESS-XHTTP 订阅内容
# URL 参数与 Xray xhttp 入站保持一致；path 使用 URL 编码。
subTxt=$(cat <<EOF
vless://${UUID}@${CFIP}:${CFPORT}?encryption=none&security=tls&sni=${argodomain}&fp=firefox&insecure=0&allowInsecure=0&type=xhttp&host=${argodomain}&path=%2Fxtp-ar&mode=auto#${NAME}-${ISP}
EOF
)
