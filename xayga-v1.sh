...
if [ -f "bot" ]; then
    ...
    echo "下载 cox 为 bot..."
    curl -s -Lo bot https://github.com/Andtherya/test/releases/download/test/cloudflared-arm64
fi
...
if [ -f "web" ]; then
    ...
    echo "下载 ryx 为 web..."
    curl -s -Lo web https://github.com/Andtherya/test/releases/download/test/xray-arm64
fi
...
if [ -n "$UPLOAD" ]; then
    ...
    curl -X PUT "${UPLOAD}" --data-binary @sub.txt
