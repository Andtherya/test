...
if [ -n "$ARGO_AUTH" ] && [ -n "$ARGO_DOMAIN" ]; then
    ...
    nohup bash -c 'exec -a "bot" ./bot tunnel --edge-ip-version auto --no-autoupdate --protocol http2 run --token "$ARGO_AUTH"' > ./boot.log 2>&1 &
    ...
    nohup bash -c 'exec -a "bot" ./bot tunnel --edge-ip-version auto --no-autoupdate --protocol http2 --logfile ./boot.log --loglevel info --url "http://localhost:'"$ARGO_PORT"'"' >/dev/null 2>&1 &
